...
describe("inflation components", () => {
  ...
  test("adds phase 1 CPI components only in expanded view without fake contributions", () => {
  ...
  test("preserves null contribution treatment when deriving live values", () => {
    ...
    expect(live.index).toBe(114);
    expect(live.contribution).toBeNull();
    expect(live.contributionEligible).toBe(false);
  });
});
