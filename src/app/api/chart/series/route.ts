...
interface GoldObsRow { series_id: string; date: string; value: number }
interface GoldEquityRow { ticker: string; date: string; total_return_index: number; price_return_index: number | null }
...
export async function GET(req: Request) {
      ...
      try {
        ...
        if (rows.length) {
          const obs = rows.map((r) => ({
            date: r.date,
            value: reqUnits === "price" ? (r.price_return_index ?? r.total_return_index) : r.total_return_index,
          }));
        ...
        if (fredRows.length) {
          return json({ source: "DB", id, label: id, observations: fredRows.map((r) => ({ date: r.date, value: r.value })) });
        }
      } catch (err) {
  ...
  if (goldEnabled()) {
    try {
      ...
      if (rows.length) {
        return json({ source: "DB", id, label: meta?.label ?? id, units, observations: rows.map((r) => ({ date: r.date, value: r.value })) });
      }
    } catch (err) {
